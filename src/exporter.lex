...
type Config = { endpoint :: Str, service :: Str }

fn stdout_config(service :: Str) -> Config {
  { endpoint: "", service: service }
}

fn otlp_config(endpoint :: Str, service :: Str) -> Config {
...
fn attrs_json(attrs :: List[(Str, Str)]) -> jv.Json {
  JList(list.map(attrs, fn (pair :: (Str, Str)) -> jv.Json {
    match pair {
      (k, v) => kv_str(k, v),
    }
  }))
...
fn resource_json(service :: Str) -> jv.Json {
  JObj([("attributes", JList([kv_str("service.name", service)]))])
}
...
fn ms_to_nano_str(ms :: Int) -> Str
  examples {
    ms_to_nano_str(0) => "0",
    ms_to_nano_str(1) => "1000000",
    ms_to_nano_str(1700000000000) => "1700000000000000000"
  }
{
...
fn log_record_json(r :: log.LogRecord) -> jv.Json {
  JObj([("timeUnixNano", JStr(ms_to_nano_str(r.ts_ms))), ("severityNumber", JInt(log.level_num(r.level))), ("severityText", JStr(log.level_text(r.level))), ("body", JObj([("stringValue", JStr(r.body))])), ("attributes", attrs_json(r.attrs)), ("traceId", JStr(r.trace_id)), ("spanId", JStr(r.span_id))])
}

fn logs_payload(service :: Str, records :: List[log.LogRecord]) -> Str {
  jv.stringify(JObj([("resourceLogs", JList([JObj([("resource", resource_json(service)), ("scopeLogs", JList([JObj([("scope", scope_json()), ("logRecords", JList(list.map(records, fn (r :: log.LogRecord) -> jv.Json {
    log_record_json(r)
  })))])]))])]))]))
}
...
fn span_json(sp :: span.Span) -> jv.Json {
  JObj([("traceId", JStr(sp.ctx.trace_id)), ("spanId", JStr(sp.ctx.span_id)), ("parentSpanId", JStr(sp.parent_id)), ("name", JStr(sp.name)), ("startTimeUnixNano", JStr(ms_to_nano_str(sp.start_ms))), ("endTimeUnixNano", JStr(ms_to_nano_str(sp.end_ms))), ("attributes", attrs_json(sp.attrs)), ("status", span_status_json(sp.status))])
}

fn traces_payload(service :: Str, spans :: List[span.Span]) -> Str {
  jv.stringify(JObj([("resourceSpans", JList([JObj([("resource", resource_json(service)), ("scopeSpans", JList([JObj([("scope", scope_json()), ("spans", JList(list.map(spans, fn (sp :: span.Span) -> jv.Json {
    span_json(sp)
  })))])]))])]))]))
}
...
fn metric_json(m :: meter.Metric, ts_ms :: Int) -> jv.Json {
  match m {
    Counter(name, delta, attrs) => JObj([("name", JStr(name)), ("sum", JObj([("dataPoints", JList([JObj([("asInt", JStr(int.to_str(delta))), ("attributes", attrs_json(attrs)), ("timeUnixNano", JStr(ms_to_nano_str(ts_ms)))])])), ("isMonotonic", JBool(true)), ("aggregationTemporality", JInt(2))]))]),
    Gauge(name, value, attrs) => JObj([("name", JStr(name)), ("gauge", JObj([("dataPoints", JList([JObj([("asDouble", JFloat(value)), ("attributes", attrs_json(attrs)), ("timeUnixNano", JStr(ms_to_nano_str(ts_ms)))])]))]))]),
    Histogram(name, value, attrs) => JObj([("name", JStr(name)), ("histogram", JObj([("dataPoints", JList([JObj([("sum", JFloat(value)), ("count", JStr("1")), ("attributes", attrs_json(attrs)), ("timeUnixNano", JStr(ms_to_nano_str(ts_ms)))])])), ("aggregationTemporality", JInt(2))]))]),
  }
}

fn metrics_payload(service :: Str, metrics :: List[meter.Metric], ts_ms :: Int) -> Str {
  jv.stringify(JObj([("resourceMetrics", JList([JObj([("resource", resource_json(service)), ("scopeMetrics", JList([JObj([("scope", scope_json()), ("metrics", JList(list.map(metrics, fn (m :: meter.Metric) -> jv.Json {
    metric_json(m, ts_ms)
  })))])]))])]))]))
}
...
fn post_otlp(endpoint :: Str, path :: Str, body :: Str) -> [net] Unit {
  ...
  let __lex_discard_1 := http.post(url, bytes.from_str(body), "application/json")
  ()
}
