...
type Span = {
  ctx         :: ctx.TraceCtx,
  parent_id   :: Str,           # "" if root span
  name        :: Str,
  service     :: Str,
  attrs       :: List[(Str, Str)],
  start_ms    :: Int,
  end_ms      :: Int,           # 0 until finish() is called
  status      :: SpanStatus,
}
...
fn finish(sp :: Span) -> [time] Span {
  { ctx: sp.ctx, parent_id: sp.parent_id, name: sp.name, service: sp.service,
    attrs: sp.attrs, start_ms: sp.start_ms, end_ms: time.now_ms(), status: sp.status }
}

fn set_attr(sp :: Span, key :: Str, value :: Str) -> Span {
  { ctx: sp.ctx, parent_id: sp.parent_id, name: sp.name, service: sp.service,
    attrs: list.append(sp.attrs, [(key, value)]), start_ms: sp.start_ms,
    end_ms: sp.end_ms, status: sp.status }
}

fn set_error(sp :: Span, msg :: Str) -> Span {
  { ctx: sp.ctx, parent_id: sp.parent_id, name: sp.name, service: sp.service,
    attrs: sp.attrs, start_ms: sp.start_ms, end_ms: sp.end_ms, status: SpanError(msg) }
}

fn duration_ms(sp :: Span) -> Int {
  if sp.end_ms > 0 { sp.end_ms - sp.start_ms } else { 0 }
}
